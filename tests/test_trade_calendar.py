...
def _cal(df: pd.DataFrame) -> TradeCalendar:
    """创建一个 TradeCalendar，其 fetcher.fetch_trade_cal 按日期范围过滤返回数据。"""
...
def test_is_trade_date_true():
    cal = _cal(_trade_cal_df([("2026-03-09", True)]))
    assert cal.is_trade_date(date(2026, 3, 9)) is True


def test_is_trade_date_false_weekend():
    cal = _cal(_trade_cal_df([("2026-03-08", False)]))
    assert cal.is_trade_date(date(2026, 3, 8)) is False


def test_is_trade_date_empty_calendar():
    cal = _cal(pd.DataFrame())
    assert cal.is_trade_date(date(2026, 3, 8)) is False
...
def test_last_trade_date_of_prev_month_normal():
    ...
    cal = _cal(_trade_cal_df(FEB_MAR_2026))
    result = cal.last_trade_date_of_prev_month(date(2026, 3, 10))
...
def test_last_trade_date_of_prev_month_january():
    ...
    cal = _cal(_trade_cal_df(dec_jan))
    result = cal.last_trade_date_of_prev_month(date(2026, 1, 15))
...
def test_last_trade_date_of_prev_month_ref_is_first_day():
    ...
    cal = _cal(_trade_cal_df(FEB_MAR_2026))
    result = cal.last_trade_date_of_prev_month(date(2026, 3, 1))
    assert result == date(2026, 2, 27)
...
def test_last_trade_date_of_month_normal():
    cal = _cal(_trade_cal_df(FEB_MAR_2026))
    result = cal.last_trade_date_of_month(date(2026, 2, 15))
    assert result == date(2026, 2, 27)
...
def test_last_trade_date_of_month_december():
    ...
    cal = _cal(_trade_cal_df(dec_data))
    result = cal.last_trade_date_of_month(date(2026, 12, 20))
...
def test_prev_trade_date_normal():
    ...
    cal = _cal(_trade_cal_df(
        [("2026-03-10", True)],
        pretrade={"2026-03-10": "2026-03-09"},
    ))
    result = cal.prev_trade_date(date(2026, 3, 10))
...
def test_prev_trade_date_raises_when_no_calendar():
    ...
    with pytest.raises(ValueError, match="交易日历中未找到"):
        cal.prev_trade_date(date(2026, 3, 10))
